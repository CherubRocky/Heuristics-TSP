...
\begin{abstract}
El presente trabajo es un reporte acerca de una implementación de Aceptación por Umbrales, una variante de la Heurística de Recocido Simulado, específicamente para la resolución del Problema del Agente Viajero (TSP) en su variante de Camino Hamiltoniano. Para el desempeño de la implementación, se diseñó un estudio experimental sobre la instancia de 150 ciudades ejecutando una muestra de 1000 semillas pseudoaleatorias. La mejor solución encontrada para dicha instancia fue de 0.139402 en un tiempo aproximado de 50s.
\end{abstract}
...
La heurística fue programada en el lenguaje Go.

Se implementaron los algoritmos proporcionados por el profesor Canek Peláez Valdés \cite{notas_ta}, así como el modelo de aristas no existentes utilizando su distancia natural —calculada a partir del radio de la tierra— multiplicada por la distancia máxima de la instancia para forzar soluciones factibles \cite{notas_tsp}.

Además, se hicieron las siguientes optimizaciones:
...
\begin{itemize}
    ...
    \item \textbf{Parámetros Experimentales:} Se utilizaron lotes de 5,000 iteraciones por nivel térmico, un factor de enfriamiento $\phi = 0.95$ y una tolerancia $\epsilon = 0.0001$. Se procesaron 1,000 semillas concurrentemente utilizando \textit{goroutines}.
\end{itemize}
...
\bibitem{notas_ta} 
Peláez, C. (s.f.) 
\textit{Notas de clase sobre recocido simulado y aceptación por Umbrales}. 
Universidad Nacional Autónoma de México.

\bibitem{notas_tsp} 
Peláez, C. (s.f.) 
\textit{Notas de clase sobre el Problema del Agente Viajero y aceptación por Umbrales}. 
Universidad Nacional Autónoma de México.
